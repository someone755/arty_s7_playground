...
reg	[APP_DATA_WIDTH-1:0]		app_wdf_data =	128'b0;	// Data for write commands, 64-wide
reg								app_wdf_end =	1'b1;	// Active high indicates current clock cycle is last cycle of input data on app_wdf_data[]
wire	[APP_MASK_WIDTH-1:0]	app_wdf_mask = 	16'b0;	// Input; Mask for app_wdf_data[], 8-wide
...
reg r_uart_rx_128_done = 1'b0; // pulses high for one clk; signals 16 bytes received to ddr block
reg [1:0] r2_rx_state = 2'b0;
reg [2:0] r3_uart_state = 3'b0; // state machine select
reg [3:0] r4_uart_byte_index = 4'b0; // counts 16 bytes to send to DDR
reg [127:0] r128_rx_to_ddr = 128'b0; // 128 bit buffer to DDR
reg [127:0] r128_ddr_rd_buffer = 128'b0; // 128 bit (read) buffer from DDR
reg [127:0] r128_2_rx_buff [0:1];
reg [3:0] r4_rx_byte_index = 4'b0; // counts 16 bytes for 64x2-bit RX vector
reg r1_rx_word_index = 1'b0; // counts 128 bit words in 64x2-bit RX 
reg r1_rx_word_index_prev;
...
always @(posedge ui_clk) begin: rx_state_machine
	if (w_uart_rx_done) begin
		...
		if (r4_rx_byte_index == 4'b1111) begin
			...
			r_uart_rx_128_done <= 1;
		end
	end
	r_uart_rx_128_done <= 0;
	r_uart_rx_en <= 1'b1;
...
case (r3_uart_state)
	...
	'b001: begin // SIGNAL RD CMD TO DDR BLOCK
		if (app_rdy & app_wdf_rdy & init_calib_complete) begin
			...
			app_en <= 1;
			app_wdf_wren <= 1;
			app_wdf_data <= r128_2_rx_buff[~r1_rx_word_index]; // data to write to DDR
	...
	'b010: begin // SIGNAL TO UART MASTER: 8 BYTES WRITTEN TO DDR
		...
		if (~app_en & ~app_wdf_wren) begin
			...
			r4_uart_byte_index <= 4'b0; // reset byte counter
			app_addr <= app_addr + 8; // increment ddr addr 128 bits
